...
Alternatively, consider installing with [[https://github.com/progfolio/elpaca][elpaca]] or [[https://www.gnu.org/software/emacs/manual/html_node/use-package/Install-package.html][package-vc]]
...
*** Executables
You can customize whether executables in path are suggested if no history
entries match using the custom variable ~esh-autosuggest-executables~, which
defaults to ~t~.
